...
async def lifespan(app: FastAPI):
    ...
    # 确保必要的集合和索引存在
    try:
        from core.database import ensure_collections
        await ensure_collections()
        logger.info("数据库索引检查完成")
    except Exception as e:
        logger.warning(f"数据库索引创建警告: {e}")
...
try:
    ...
    app.include_router(chat_router, prefix="/api/v1/chat", tags=["智能助手"])
    logger.info("✅ 智能助手路由已加载")
except ImportError as e:
    logger.warning(f"智能助手路由未加载: {e}")

# 注意：目前没有单独的 agent API，智能体功能通过 chat API 提供
