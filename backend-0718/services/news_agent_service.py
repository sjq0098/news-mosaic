...
def get_available_models() -> List[str]:
    ...
    Returns:
        List[str]: 可用模型列表
    """
    return settings.AVAILABLE_AI_MODELS.copy()
...
async def get_news_agent_service(model_name: str = None) -> "NewsAgentService":
    ...
    # 确定使用的模型名称
    effective_model = model_name or settings.DEFAULT_AI_MODEL
...
请分析以下用户输入："""
    ...
    # 关键词和时间联合提取提示词
    KEYWORDS_TIME_EXTRACTION_PROMPT = """你是一个智能信息提取助手，任务是从用户的新闻搜索请求中同时提取关键词和时间信息。
...
请分析以下用户输入："""
    ...
    # 时间提取提示词 - 增强版
    TIME_EXTRACTION_PROMPT = """你是一个时间信息提取助手，任务是从用户的新闻搜索请求中提取时间相关信息。
...
请分析以下用户输入并提取时间信息："""
    ...
    def __init__(self, model_name: str = None) -> None:
        ...
        # 验证模型是否可用
        if self.model_name not in settings.AVAILABLE_AI_MODELS:
            logger.warning(f"模型 {self.model_name} 不在可用列表中，使用默认模型 {settings.DEFAULT_AI_MODEL}")
...
💡 **提示**: 如果您对某个领域特别感兴趣，可以告诉我具体的关键词！"""
    ...
    async def _extract_time_period(self, user_message: str) -> str:
        ...
        try:
            messages = [
                SystemMessage(content=self.TIME_EXTRACTION_PROMPT),
                HumanMessage(content=user_message)
            ]
    ...
    async def _manage_interests(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if not operations_performed:  # 只有在没有其他操作时才检查复合操作
                additional_keywords = await self._extract_keywords_from_message(user_message)
...
"""
    ...
    async def _extract_time_period(self, user_message: str) -> str:
        ...
        try:
            ...
            logger.error(f"时间提取失败: {str(e)}")
            print(f"❌ [时间提取] 失败，使用默认时间: 1w")
            return "1w"
