...
class NewsService:
    ...
    # 类常量
    DEFAULT_EXPIRE_DAYS = 7  # 修改为7天，即1周
    ...
    async def search_and_save_news(self, request: NewsSearchRequest) -> NewsSearchResult:
        ...
        try:
            ...
            # 根据请求参数确定清理天数
            # 优先使用请求中的expire_days，否则根据时间参数计算
            if hasattr(request, 'expire_days') and request.expire_days:
                cleanup_days = request.expire_days
            else:
                cleanup_days = self._get_cleanup_days_from_time_period(request.time_period)
            
            # 搜索前自动清理过期新闻（使用动态天数）
            await self._auto_cleanup_expired_news(request.session_id, cleanup_days)
    ...
    async def refresh_expired_news(self, session_id: str, expire_days: int = DEFAULT_EXPIRE_DAYS, time_period: str = "1w") -> NewsRefreshResult:
        """
        刷新指定会话的过期新闻
        ...
        try:
            ...
            if expired_data["keywords"] and self.serpapi_key:
                new_articles_count = await self._refresh_with_keywords(
                    session_id, expired_data["keywords"], time_period
                )
    ...
    def _get_cleanup_days_from_time_period(self, time_period: str) -> int:
        """根据时间参数获取清理天数
        
        Args:
            time_period: 时间参数 (1d, 1w, 1m, 1y)
        ...
        # 如果是有效的时间参数，返回对应天数；否则返回默认值
        return time_mapping.get(time_period, self.DEFAULT_EXPIRE_DAYS)
    ...
    async def _refresh_with_keywords(self, session_id: str, keywords: List[str], time_period: str = "1w") -> int:
        """用关键词重新搜索新闻
        
        Args:
            session_id: 会话ID
            keywords: 关键词列表
            time_period: 时间范围，默认1周
        """
        new_articles_count = 0
        ...
        for batch in keyword_batches[:3]:  # 限制批次避免API配额超限
            try:
                request = NewsSearchRequest(
                    ...
                    keywords=batch,
                    num_results=5,
                    time_period=time_period  # 使用传入的时间参数
                )
