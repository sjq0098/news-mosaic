...
// 统一新闻处理API
export const unifiedNewsApi = {
  // 统一新闻处理
  processUnified: (params: {
    query: string
    num_results?: number
    language?: string
    country?: string
  ...
  }) => api.post('/api/unified-news/process', params),

  // 快速搜索
  quickSearch: (query: string) => api.post('/api/unified-news/quick-search', null, {
    params: { query }
  }),
  ...
  searchAndAnalyze: (params: {
    ...
    include_sentiment?: boolean
    include_summary?: boolean
  }) => api.post('/api/unified-news/search-and-analyze', params),

  // 健康检查
  healthCheck: () => api.get('/api/unified-news/health'),
}
